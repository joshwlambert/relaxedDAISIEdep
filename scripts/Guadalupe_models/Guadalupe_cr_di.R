# Guadalupe constant rate diversity-independent model
...
Guadalupe_cr_di_max_loglik <- Guadalupe_cr_di[[max_loglik]]

#Calculate BIC
Guadalupe_cr_di_BIC <-
  4 * (log(1000) + log(2 * pi)) - 2 * Guadalupe_cr_di[[max_loglik]]$loglik

#Save results
results_file_path <- file.path(getwd(), "results", "Guadaplupe",
                               "Guadalupe_cr_di.RData")
Guadalupe_cr_di_results <- list(
  Guadalupe_cr_di_max_loglik = Guadalupe_cr_di_max_loglik,
  Guadalupe_cr_di_BIC = Guadalupe_cr_di_BIC)

save(Guadalupe_cr_di_results,
