# Hawaii constant rate diversity-independent model
...
Hawaii_cr_di_max_loglik <- Hawaii_cr_di[[max_loglik]]

#Calculate BIC
Hawaii_cr_di_BIC <-
  4 * (log(1000) + log(2 * pi)) - 2 * Hawaii_cr_di[[max_loglik]]$loglik

#Save results
results_file_path <- file.path(getwd(), "results", "Hawaii",
                               "Hawaii_cr_di.RData")
Hawaii_cr_di_results <- list(
  Hawaii_cr_di_max_loglik = Hawaii_cr_di_max_loglik,
  Hawaii_cr_di_BIC = Hawaii_cr_di_BIC)

save(Hawaii_cr_di_results,
